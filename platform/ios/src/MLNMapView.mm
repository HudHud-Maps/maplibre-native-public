...
- (void)removeLocationIndicatorLayer {
  if (self.userLocationAnnotationView &&
      [self.userLocationAnnotationView
          isKindOfClass:[MLNLocationIndicatorUserLocationAnnotationView class]]) {
...
- (void)createUserLocationAnnotationView {
  ...
  if ([self.delegate respondsToSelector:@selector(mapView:viewForAnnotation:)]) {
    ...
    if (userLocationAnnotationView &&
      ...
      [NSException
           raise:MLNUserLocationAnnotationTypeException
          format:
              @"User location annotation view must be a kind of MLNUserLocationAnnotationView. %@",
  ...
  self.userLocationAnnotationView =
      userLocationAnnotationView
          ?: self.useLocationIndicatorLayer
             ? [[MLNLocationIndicatorUserLocationAnnotationView alloc] init]
             : [[MLNFaux3DUserLocationAnnotationView alloc] init];
...
- (MLNPluginLayerTileFeature *)featureFromCore:(std::shared_ptr<mbgl::plugin::Feature>)feature {
  ...
  for (auto &coordinateCollection : feature->_featureCoordinates) {
    for (auto &coordinate : coordinateCollection._coordinates) {
      MLNPluginLayerFeatureCoordinate *featureCoordinate =
          [[MLNPluginLayerFeatureCoordinate alloc] init];
...
- (NSString *)tileIDToString:(mbgl::OverscaledTileID &)tileID {
  NSString *tempResult = [NSString
      stringWithFormat:@"%i,%i,%i", tileID.canonical.z, tileID.canonical.x, tileID.canonical.y];
  return tempResult;
}
...
- (void)addPluginLayerType:(Class)pluginLayerClass {
  ...
  factory->setOnLayerCreatedEvent([layerClass, weakMapView,
    ...
    if (capabilities.supportsReadingTileFeatures) {
      pluginLayerImpl->setFeatureCollectionLoadedFunction(
          [weakPlugInLayer,
           weakMapView](const std::shared_ptr<mbgl::plugin::FeatureCollection> featureCollection) {
            @autoreleasepool {
              MLNMapView *strongMapView = weakMapView;
              if (!strongMapView) {
                return;
              ...
              MLNPluginLayerTileFeatureCollection *collection =
                  [[MLNPluginLayerTileFeatureCollection alloc] init];

              // Add the features
              NSMutableArray *featureList =
                  [NSMutableArray arrayWithCapacity:featureCollection->_features.size()];
              ...
              collection.tileID =
                  [strongMapView tileIDToString:featureCollection->_featureCollectionTileID];
              ...
              collection.sprites.texture =
                  [MLNTexture textureWithTexture:featureCollection->_spriteAtlas];
              ...
              collection.glyphs.texture =
                  [MLNTexture textureWithTexture:featureCollection->_glyphAtlas];

              [weakPlugInLayer onFeatureCollectionLoaded:collection];
            }
          });

      pluginLayerImpl->setFeatureCollectionUnloadedFunction(
          [weakPlugInLayer,
           weakMapView](const std::shared_ptr<mbgl::plugin::FeatureCollection> featureCollection) {
            @autoreleasepool {
              MLNMapView *strongMapView = weakMapView;
    ...
    pluginLayerImpl->setSpritePropertiesFunction(
        [weakPlugInLayer]() -> std::vector<mbgl::plugin::FeatureSymbolProperty> {
          @autoreleasepool {
            ...
            for (MLNPluginLayerFeatureSymbolProperty *propertyObject in spriteProperties) {
              mbgl::plugin::FeatureSymbolProperty::Type type =
                  mbgl::plugin::FeatureSymbolProperty::Type::LITERAL;
    ...
    pluginLayerImpl->setGlphyPropertiesFunction(
        [weakPlugInLayer]() -> std::vector<mbgl::plugin::FeatureSymbolProperty> {
          // TODO(yousifd): Add more properties to the FeatureGlyphProperty to modify how the text
          // is shaped
...
- (void)addPluginProtocolHandler:(Class)pluginProtocolHandlerClass {
  ...
  std::shared_ptr<mbgl::PluginFileSource> pluginSource =
      std::make_shared<mbgl::PluginFileSource>(resourceOptions, clientOptions);
  pluginSource->setOnRequestResourceFunction(
      [weakHandler, weakSelf](const mbgl::Resource &resource) -> mbgl::Response {
        ...
        if (strongHandler) {
          ...
          // TODO: Figure out what other fields in response need to be passed back from
          // requestResource
  ...
  pluginSource->setOnCanRequestFunction(
      [weakHandler, weakSelf](const mbgl::Resource &resource) -> bool {
        @autoreleasepool {
          __strong MLNPluginProtocolHandler *strongHandler = weakHandler;
          if (!strongHandler) {
            return false;
